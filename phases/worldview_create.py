...
与えられた長文説明から、重要な固有名詞を抽出・命名し、スキーマに従って JSON 配列で返してください。
...
【命名ルール】
- ルビは必要な場合のみ付与し、丸括弧ではなく必ず 〈 〉 を用いる（例：星辰議会〈セレスティアルコード〉）。
- すべてにルビは付けない。初出の重要語・読みが難しい語・異言語由来語のみ付ける。
- 直訳は避け、世界観の文化・宗教観・地理・音韻に合う“少しひねった”固有名にする。
- 既存の一般名詞/記号的名称（「古代寺院」「古文書」など）だけにしない。必ず固有名化（例：古代寺院 → 黎明環堂〈ドーンリング〉）。
- 国家/宗教/軍事組織は接尾辞・接頭辞の体系を意識して統一感を持たせる（例：〜連邦、〜王国、〜教団、〜騎士団）。
- 地名は地勢・由来・機能が想起できる語構成にする（例：霧縁の環礁、斑雪峠〈はんせつとうげ〉）。
- 人名/NPC名は文化圏の音韻・命名規則に寄せ、役割や肩書は後置。
    例：
        - ユルグ・ローヴァン／灰眼の査閲官
        - アシマ・ザフリ／紅蓮の隊長
        - ルィサ・オルメ／黄砂の占星師
- 種族名は“分類群＋異称”の二段構えを許容。
    例：
        - 空殻の民〈ホロウフォーク〉
        - 樹皮の旅人（バークウォーカー）
        - 星影を纏う民〈スターシェイド〉
...
- 伝承・神話は抽象概念＋出来事。
    例：
        - 灰冠の予言〈グレイ・クラウン・プロフェシー〉
        - 連弩の夜
        - 七潮の約束〈セブンタイド・パクト〉
- 類似名の乱立を避け、セッション内での識別性・再利用性を優先する。
- 既存IPの名称・紛らわしい著名名称は使用しない。
