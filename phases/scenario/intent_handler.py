...
プレイヤーとの対話を通じて、世界を描写し、特に**PCの置かれている状況の明記**に気をつけ、状況に応じた進行を行ってください。
プレイヤーに提示する文章は、わかりやすいようあまり固有名詞を使わず（使うときは十分説明付きで）、三人称・地の文で常体にしてください。描写はかっこつけず、具体性とわかりやすさを優先してください。
プレイヤーが操作するキャラクターをプレイヤーキャラクター（以後PC）と呼びます。
...
キャラクターの現状をちゃんと把握できるように、わかりやすさを優先して描写してください。**セクションの進行忘れには注意してください**
""",
...
以下返答用情報
・このシステムにセーブや中断と言った機能はありません。基本的に常時ログとして保存され、再度開始時に自動でログから復元されます。
...
以下のルールに従ってください：
- プレイヤーが望まない限り、連続しての行為判定は行わないでください。
- 判定を促す文は「〜行為判定を行います。」という形で必ず終えてください。
- **技能の種類や難易度、成功条件などは絶対に提示しないでください。**
- 文章の末尾に `[action_check]` を追加してください。
- 行為判定後は、十分にシナリオを進行させてください。

### 例（出力形式）：

---
君は壁越しに音を立てず近づき、様子を探る。
では、壁越しに気配を察知できるかどうか、行為判定を行います。[action_check]
---
...
""",
    "end_section": """
【重要】GM用メモの「のセクションの目的」を達成し現在のセクションでやるべきことを終え、次のセクションあるいは章に進む場合 あるいはセッションを最後までやり終えてシナリオを終了する場合は、出力の最後尾に[end_section]と出力してください。**セクションの進行忘れには気をつけてください。**忘れた場合はあとからでも出力し、帳尻を合わせてください。
""",
...
class IntentHandler:
    ...
    def _handle_intro(self, kind: str) -> str:
        ...
        if kind == "chapter":
            instruction = (
                "あなたはTRPGのゲームマスターです。\n"
                "以下の情報と会話履歴を参考に、新たに始まる章の導入描写を提示してください。\n"
                "プレイヤーキャラクターのこれまでを、非常に簡潔に伝えつつ、今現在どこにいてどのような状況なのかが分かるようにわかりやすさを優先して描写してください。\n"
                "プレイヤーキャラクターがこれから何をすれば良いのかが自然とわかるように、具体的な背景と、直後の行動の選択肢となるような情報をそれとなく含めてください。このとき、直前に描写した状況と矛盾しないよう気をつけてください。\n"
                "プレイヤーに提示する文章は、わかりやすいようあまり固有名詞を使わず（使うときは十分説明付きで）、三人称・地の文で常体にしてください。"
            )
        else:
            instruction = (
                "あなたはTRPGのゲームマスターです。\n"
                "以下の情報と会話履歴を参考に、セクションの導入描写を200字程度で提示してください。\n"
                "直前の描写と自然につながるようにし、プレイヤーキャラクターが今現在どこにいてどのような状況なのかが分かるように描写してください。\n"
                "プレイヤーキャラクターがこれから何をすれば良いのかが自然とわかるように、具体的な背景と、直後の行動の選択肢となるような情報をそれとなく含めてください。このとき、直前に描写した状況と矛盾しないよう気をつけてください。\n"
                "プレイヤーに提示する文章は、わかりやすいようあまり固有名詞を使わず（使うときは十分説明付きで）、三人称・地の文で常体にしてください。"
            )
    ...
    def _handle_exploration(self, label: str, player_input: str) -> str:
        ...
        if label == "action" and goal_text:  
            goal_instruction = (
                ...
                "※この目的をすべて満たした場合、出力の最後に必ず[end_section]を単独の行で書くこと。このとき、行為判定や戦闘判定を同時に行おうとしないこと。\n"
    ...
    def _render_pre_snippet(self, key: str) -> str:
        ...
        if key == "worldview":
            ...
            if goal:
                lines.append(f"このセクションの目的: {goal}")
            if desc:
    ...
    def _handle_post_check_description(self) -> str:
        system_prompt = (
            ...
            "PCが直前に行った行為判定の結果を踏まえ、特に**PCの置かれている状況の明記**に気をつけ、"
            "その行動が物語にどのような影響を与えたかを描写してください。\n"
            "プレイヤーに提示する文章は、三人称・地の文で常体にしてください。\n\n"

            "プレイヤーの入力から判定結果を受け取り、"
            "『その結果によって起こった出来事』や"
            "『状況の変化』『登場人物の反応』『環境の変化』などをそれらしく描写してください。\n"
            "おおよそ200字程度で、キャラクターの現状をちゃんと把握できるように描写しつつ、シナリオをGMメモに従って十分進行させてください。\n"

            "行為判定には出目や補正値に応じた結果の“質”があります。以下の分類に基づいて、描写のトーンを調整してください：\n"
            "- 【クリティカル】（出目12）：劇的に有利な展開や幸運な状況を描写してください。\n"
            "- 【ファンブル】（出目2）：予期せぬトラブルや危機的な事態、あるいは愚かな失策を描写してください。\n"
            ...
            "ただし、プレイヤーキャラクターは物語の途中で決して死亡しません。\n"
    ...
    def _handle_post_combat_description(self) -> str:
        system_prompt = (
            ...
            "その戦闘行動が物語にどのような影響を与えたかを描写してください。\n"
            "プレイヤーに提示する文章は、三人称・地の文で常体にしてください。\n\n"
            ...
            "・他の登場人物の反応\n"
            "などをそれらしく描写してください。\n\n"
            "おおよそ200字程度で、キャラクターの現状をちゃんと把握できるように描写してください。\n"
