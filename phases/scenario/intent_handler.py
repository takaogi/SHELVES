...
**「このセクションの目的」を常に注意して進行し、決して目的を飛ばしたり、コマンドによる進行を忘れないようにしてください。**
基本的に文量は300字程度を目安に、説明と選択肢提示で半々くらいを心がけてください。選択肢は、「このセクションの目的」を前提に考案してください（ネタバレは避ける）。
...
- 一度判定に成功したなら、基本的に同等の内容については後顧の憂いを絶ったものとしてください。
...
""",
    "end_section": """
【重要】GM用メモの「のセクションの目的」を達成し現在のセクションでやるべきことを終え、次のセクションあるいは章に進む場合
...
class IntentHandler:
    ...
    def _handle_post_check_description(self) -> str:
        system_prompt = (
            ...
            "その行動が物語にどのような影響を与え、どのようにシナリオが進行したかを描写してください。\n"
            "プレイヤーに提示する文章は、三人称・地の文で常体にしてください。\n\n"

            "プレイヤーの入力から判定結果を受け取り、"
            "『その結果によって起こった出来事』や"
            "『状況の変化』『登場人物の反応』『環境の変化』などをそれらしく描写してください。\n"
            "おおよそ300字程度で、キャラクターの現状をちゃんと把握できるように描写しつつ、シナリオをGMメモに従って**大きく**進行させてください。\n"
