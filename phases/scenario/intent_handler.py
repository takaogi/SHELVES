...
プレイヤーに提示する文章は、**わかりやすいようあまり固有名詞を使わず（使うときは十分説明付きで）**、ラノベ風の三人称・地の文で常体にしてください。描写はかっこつけず、わかりやすさを優先して説明してください。雰囲気を壊すようなメタな表現は決してしないでください。"
...
class IntentHandler:
    ...
    def _handle_post_check_description(self) -> str:
        system_prompt = (
            ...
            "ただし、これらの質の表現は描写のみで行い、達成値や目標値等の**メタな表現**は決して使わないでください。\n\n"
    ...
    def _handle_post_combat_description(self) -> str:
        system_prompt = (
            ...
            "ただし、これらの質の表現は描写のみで行い、達成値や判定結果等の**メタな表現**は決して使わないでください。\n\n"
