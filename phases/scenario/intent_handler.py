...
基本的に一度に出力する文量は300字程度を目安にし、エピローグ等の描写時やセクションエンド時等描写を強化したいときは、500字ほどを目安に記述してください。
...
セクションを終えるときを除いて、プレイヤーキャラクターがこれから何をすれば良いのかが自然とわかるように、具体的な背景と、（セクションエンド時でなければ）直後の行動の選択肢を2~3ほど含めてください。ただし、選択を迫る発言（選択してください。選べ。等）はしないでください。提示するだけで構いません。このとき、直前に描写した状況と矛盾しないよう気をつけてください。
