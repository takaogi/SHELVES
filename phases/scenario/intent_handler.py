...
**ただし、セクションエンド時（この後'[end_section]'を出力する場合）と、行為判定'[action_check]'や戦闘判定'[combat_start]'を行うときは、行動案を提示しないでください。**
...
- アイテムを追加したい場合（個数や説明も含める）：
  [command:add_item("アイテム名", 個数, "説明文")]

- アイテムを削除したい場合（個数を指定する場合は省略可）：
  [command:remove_item("アイテム名", 個数)]

※ 個数は整数で、アイテムを削除したい場合は説明文を省略可能です。既存のアイテムを追加するとき、説明文を変更しない場合は場合は空文字 "" を指定してください。

### 例：

君は箱の中から「銀の鍵」を手に取った。
→ [command:add_item("銀の鍵", 1, "古びた装飾が施された鍵")]

戦闘中に「松明」が壊れた。
→ [command:remove_item("松明", 1)]
""",
    ...
    def _render_pre_snippet(self, key: str) -> str:
        ...
        if key == "scenario":
            ...
            if items:
                ...
                for item in items:
                    if isinstance(item, str):
                        ...
                        # 新仕様: {name, count, description}
                        iname = item.get("name", "")
                        count = item.get("count", 0)
                        desc = item.get("description", "")
                        if desc:
                            item_lines.append(f"- {iname} ×{count}：{desc}")
                        else:
                            item_lines.append(f"- {iname} ×{count}")
                text += "\n所持アイテム:\n" + "\n".join(item_lines)
            ...
            for i, sec in enumerate(flow, 1):
                ...
                if i - 1 == section_idx:
                    flow_lines.append(f"▶ 第{i}セクション - 【重要！！】目的: {goal}")
                else:
                    flow_lines.append(f"  第{i}セクション - 目的: {goal}")

                if desc:
                    flow_lines.append(f"    説明: {desc}")
...
その行動が物語にどのような影響を与え、どのようにシナリオが進行したかを、
おおよそ300字程度で、雰囲気よりわかりやすさ優先で、ライトノベルに使われる程度の簡単な語彙で描写してください。
...
【セクション終了指針】
もしこのこの行為判定の成功によって、後述する「現在のセクションの目的」を達成した場合は、話に区切りをつけつつ出力の最後に**必ず** `[end_section]` を単独の行で書き、セクションを終了してください。
このとき、行動案の提示は**絶対にしないでください**。他のコマンドと同時使用可能です

【アイテム操作コマンド】
この行為判定PCの所持品に変更がある場合、出力の最後尾に以下のコマンドを記述してください（複数同時可）：
- 追加: [command:add_item("アイテム名", 個数, "説明文")]
- 削除: [command:remove_item("アイテム名", 個数)]
説明文を変更しない場合は空文字 "" を指定。

【カノン操作コマンド】
この行為判定により、既存のカノンに新たな事実を追加したり、新しいカノンを作成する場合は、
出力の最後尾に以下のコマンドを記述してください。複数同時でも構いません。（複数同時可）：
既存カノンへの追加: [command:add_history("カノン名", "追加内容")]
新規カノン作成: [command:create_canon("カノン名", "タイプ", "詳細説明")]
タイプは「場所」「NPC」「知識」「アイテム」「ギミック」「その他」から選択。
"""

        # 状況把握用の pre_snippet を付与
        pre_keys = ["scenario","worldview", "character", "nouns", "canon", "plan"]
        pre_extra = "\n\n".join(self._render_pre_snippet(k) for k in pre_keys)
        if pre_extra:
            system_prompt += "\n\n" + pre_extra
...
その戦闘行動が物語にどのような影響を与えたかを、おおよそ300字程度で、雰囲気よりわかりやすさ優先で、
ライトノベルに使われる程度の簡単な語彙で描写してください。
プレイヤーに提示する文章は、三人称・地の文で常体にしてください。
...
- 8〜11の範囲：敵の強さ、状況や戦法の質によって柔軟に判断してください。
...
これらの質の表現は描写のみで行い、達成値や判定結果、クリティカル、ファンブル等の**メタな表現**は決して使わないでください。
...
逆に、低レベルのキャラが強敵に挑む場合は、成功した場合は十分な決定打を与えても構いません。

【アイテム操作コマンド】
この戦闘描写によりPCの所持品に変更がある場合、出力の最後尾に以下のコマンドを記述してください（複数同時可）：
- 追加: [command:add_item("アイテム名", 個数, "説明文")]
- 削除: [command:remove_item("アイテム名", 個数)]
説明文を変更しない場合は空文字 "" を指定。

【カノン操作コマンド】
この戦闘描写により、既存のカノンに新たな事実を追加したり、新しいカノンを作成する場合は、
出力の最後尾に以下のコマンドを記述してください。複数同時でも構いません。（複数同時可）：
既存カノンへの追加: [command:add_history("カノン名", "追加内容")]
新規カノン作成: [command:create_canon("カノン名", "タイプ", "詳細説明")]
タイプは「場所」「NPC」「知識」「アイテム」「ギミック」「その他」から選択。
