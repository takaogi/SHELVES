...
class ChapterGenerator:
    ...
    def _output_format_sample(self) -> str:
        return (
            "- titleは「第◯章」等を含まず、シンプルかつ詩的に章の内容を表す名前のみにしてください。名前をもとの案から変更しても構いません。\n"
            "【flow補足】\n"
            "- section: セクションの順番をあらわす番号（2〜5つ、基本は3つ）。必ず連番で。章の内容と目的にあわせて、一つ一つのセクションが長くも短くもなりすぎないように分割。**あまり細かくしすぎないように、大雑把なくらいでいい**\n"
            ...
            "- goal: そのセクションを終了し、次のセクションに移る条件。プレイの自由度のために、基本的に**目的達成のための手段ではなく、目的そのものを記述してください**。全セクションを通過することで、最終的に章の目的を達成できる形にしてください。**最後のセクションのgoalは、それを達成したら直ぐに次の章に移ることを前提に、章自体の目的とほぼ一致させてください。**さらに、それが最終章なら「エピローグを終え、プレイヤーにセッション終了の確認をする」としても構いません。\n"
            ...
            "\n"
            "【canon補足】\n"
            "- この章で新たに明らかになった事実・要素を、その章において重大なもののみ最大で3つまで **絶対に既にあるカノンや世界観の固有名詞を含めてはいけない**\n"
            "- type: 以下から選んでください：\n"
            ...
            "最後に、これが最も重要ですが、地に足ついた、リアリティのあるシナリオを作ってください。"
            )
