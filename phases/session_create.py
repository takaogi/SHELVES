...
- long：8〜10章
 ...
 - medium：最終ゴールの前に「中間ゴール」を1つ設けること。
   ・中間ゴールは全体ゴールに至るための必須条件となる明確な到達点とする。
   ・章ごとのゴールは「中間ゴールまで」と「中間ゴールから最終ゴールまで」の二部構成で設計する。

 - long：最終ゴールの前に「中間ゴール」を2つ設けること。
   ・三幕構成のように「第1中間ゴール」「第2中間ゴール」を配置する。
   ・各ゴールは物語を次段階へ移す決定的な到達点とし、章ごとのゴールはそれを支える形で設計する。

 ※ 短編は中間ゴールを設けず、直接最終ゴールを逆算してよい。

■ 整合性（最重要）：
- まずシナリオ全体の "goal" を短く確定し、その後に各章の "goal" を“逆算（バックキャスティング）”で設計すること。
- 各章の "goal" は「その章を終えたと誰の目にも明確な“結果”」を一文で。手段・工程・チェックリスト化は禁止。
- 章の "goal" を 1章目から順にすべて達成すれば、論理的にシナリオの "goal" が満たされる構造にすること。
- 最終章の "goal" はシナリオ全体の "goal" と実質同義の“幕引き状態”を短く特定すること。
